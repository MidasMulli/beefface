...
**Machine patterns and the splitter cost function** - `ZinNEPatterns::NEConv::Fuse` references `dma_conv_input` and `dma_conv_output`, the first and last stages of the 17-stage pipeline, so its DMA-symbol condition is a check on those pipeline symbols. The same function carries `ane_layer`, `condition_layer` and a td-branching error string, so task-descriptor branching fusion happens inside it. `ZinMirKernelSizeSplitterEngine` scores splits on utilization and total pass count, both pure shape arithmetic with no threshold constants; 28 `ZinIrHal*::GetParams()` targets supply per-hardware parameters. See [`docs/ZIN_ENGINE_CLASSES.md`](docs/ZIN_ENGINE_CLASSES.md).
